	...
	i2c1.i2cWrite(
		...
		(err, bytesWritten, buffer) => {
			...
			assert.strictEqual(
				bytesWritten,
				cmdSetTL.length,
				"expected i2cWrite to write 3 bytes",
			);
			assert.strictEqual(
				cmdSetTL,
				buffer,
				"expected cmdSetTL to be passed to i2cWrite callback",
			);
				...
				i2c1.i2cWrite(
					...
					(err, bytesWritten, buffer) => {
						...
						assert.strictEqual(
							bytesWritten,
							cmdGetTL.length,
							"expected i2cWrite to write 1 byte",
						);
						assert.strictEqual(
							cmdGetTL,
							buffer,
							"expected cmdGetTL to be passed to i2cWrite callback",
						);
						...
						i2c1.i2cRead(DS1621_ADDR, 2, tl, (err, bytesRead, buffer) => {
							...
							assert.strictEqual(
								bytesRead,
								2,
								"expected i2cRead to read 2 bytes",
							);
							assert.strictEqual(
								tl.readUInt16LE(0),
								25,
								"expected i2cRead to read value 25",
							);
							assert.strictEqual(
								tl,
								buffer,
								"expected tl to be passed to i2cRead callback",
							);
	...
	i2c1.writeI2cBlock(
		...
		(err, bytesWritten, buffer) => {
			...
			assert.strictEqual(
				bytesWritten,
				2,
				"expected writeI2cBlock to write 2 bytes",
			);
			assert.strictEqual(
				newtl,
				buffer,
				"expected newtl to be passed to writeI2cBlock callback",
			);
				...
				i2c1.readI2cBlock(
					...
					(err, bytesRead, buffer) => {
						...
						assert.strictEqual(
							bytesRead,
							2,
							"expected readI2cBlock to read 2 bytes",
						);
						assert.strictEqual(
							buffer.readUInt16LE(0),
							22,
							"expected readI2cBlock to read value 22",
						);
						assert.strictEqual(
							newtl,
							buffer,
							"expected newtl to be passed to readI2cBlock callback",
						);
...
const readWriteWord = (_) => {
	...
	i2c1.readWord(DS1621_ADDR, CMD_ACCESS_TL, (err, oldtl) => {
		...
		assert(
			typeof oldtl === "number" && oldtl <= 0xffff,
			"expeted readWord to read a word",
		);
		...
		i2c1.writeWord(DS1621_ADDR, CMD_ACCESS_TL, newtl, (err) => {
			...
			i2c1.readWord(DS1621_ADDR, CMD_ACCESS_TL, (err, newtl2) => {
				...
				assert(
					typeof newtl2 === "number" && newtl2 <= 0xffff,
					"expeted readWord to read a word",
				);
...
const sendReceiveByte = (epectedConfig) => {
	...
	i2c1.sendByte(DS1621_ADDR, CMD_ACCESS_CONFIG, (err) => {
		...
		i2c1.receiveByte(DS1621_ADDR, (err, config) => {
			...
			assert(
				typeof config === "number" && config <= 0xff,
				"expeted receiveByte to receive a byte",
			);
			assert.strictEqual(
				config,
				epectedConfig,
				"1st and 2nd config read differ",
			);
	...
	i2c1.writeByte(DS1621_ADDR, CMD_ACCESS_CONFIG, 0x0, (err) => {
		...
		waitForWrite((_) => {
			i2c1.readByte(DS1621_ADDR, CMD_ACCESS_CONFIG, (err, config) => {
				...
				assert(
					typeof config === "number" && config <= 0xff,
					"expeted readByte to read a byte",
				);
