...
const openPromisified = (busNumber, options) =>
	new Promise((resolve, reject) => {
		const bus = open(busNumber, options, (err) =>
			err ? reject(err) : resolve(bus.promisifiedBus()),
		);
	});
...
const parseId = (id) => {
	...
	const name =
		known !== undefined ? known.name : "<0x" + manufacturer.toString(16) + ">";
...
class I2cFuncs {
	constructor(i2cFuncBits) {
		...
		this.smbusBlockProcCall = !!(
			i2cFuncBits & i2c.I2C_FUNC_SMBUS_BLOCK_PROC_CALL
		);
		...
		this.smbusWriteBlock = !!(
			i2cFuncBits & i2c.I2C_FUNC_SMBUS_WRITE_BLOCK_DATA
		);
		this.smbusReadI2cBlock = !!(
			i2cFuncBits & i2c.I2C_FUNC_SMBUS_READ_I2C_BLOCK
		);
		this.smbusWriteI2cBlock = !!(
			i2cFuncBits & i2c.I2C_FUNC_SMBUS_WRITE_I2C_BLOCK
		);
	}
}
...
class Bus {
	...
	i2cFuncsSync() {
		if (!this.funcs) {
			this.funcs = Object.freeze(
				new I2cFuncs(i2c.i2cFuncsSync(peripheralSync(this, 0))),
			);
		}
	...
	readI2cBlockSync(addr, cmd, length, buffer) {
		...
		return i2c.readI2cBlockSync(
			peripheralSync(this, addr),
			cmd,
			length,
			buffer,
		);
	...
	scanSync(startAddr, endAddr) {
		...
		const scanBus = openSync(this._busNumber, {
			forceAccess: this._forceAccess,
		});
		const addresses = [];
...
class PromisifiedBus {
	...
	readByte(addr, cmd) {
		return new Promise((resolve, reject) =>
			this._bus.readByte(addr, cmd, (err, byte) =>
	...
	readWord(addr, cmd) {
		return new Promise((resolve, reject) =>
			this._bus.readWord(addr, cmd, (err, word) =>
	...
	readBlock(addr, cmd, buffer) {
		return new Promise((resolve, reject) =>
			this._bus.readBlock(addr, cmd, buffer, (err, bytesRead, buffer) =>
	...
	receiveByte(addr) {
		return new Promise((resolve, reject) =>
			this._bus.receiveByte(addr, (err, byte) =>
				err ? reject(err) : resolve(byte),
			),
		);
	}
	...
	sendByte(addr, byte) {
		return new Promise((resolve, reject) =>
			this._bus.sendByte(addr, byte, (err) => (err ? reject(err) : resolve())),
		);
	}

	writeByte(addr, cmd, byte) {
		return new Promise((resolve, reject) =>
			this._bus.writeByte(addr, cmd, byte, (err) =>
	...
	writeWord(addr, cmd, word) {
		return new Promise((resolve, reject) =>
			this._bus.writeWord(addr, cmd, word, (err) =>
				err ? reject(err) : resolve(),
			),
		);
	}
	...
	writeQuick(addr, bit) {
		return new Promise((resolve, reject) =>
			this._bus.writeQuick(addr, bit, (err) => (err ? reject(err) : resolve())),
		);
	}
	...
	i2cRead(addr, length, buffer) {
		return new Promise((resolve, reject) =>
			this._bus.i2cRead(addr, length, buffer, (err, bytesRead, buffer) =>
				err ? reject(err) : resolve({ bytesRead: bytesRead, buffer: buffer }),
			),
		);
	}
	...
	i2cWrite(addr, length, buffer) {
		return new Promise((resolve, reject) =>
			this._bus.i2cWrite(addr, length, buffer, (err, bytesWritten, buffer) =>
				err
					? reject(err)
					: resolve({ bytesWritten: bytesWritten, buffer: buffer }),
			),
	...
	scan(...args) {
		return new Promise((resolve, reject) =>
			this._bus.scan(...args, (err, devices) =>
